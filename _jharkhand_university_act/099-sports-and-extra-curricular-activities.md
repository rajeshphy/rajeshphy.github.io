---
layout: "act"
title: "Sports and Extra- Curricular Activities"
act_part: "CHAPTER VII: ADMISSION, EXAMINATION, EVALUATION AND OTHER MATTERS RELATING TO STUDENTS - Section 99"
order: 109
source_pdf: "/assets/pdf/ref/acts/2026-Gazette-Jharkhand-University-Act.pdf"
chapter_title: "CHAPTER VII: ADMISSION, EXAMINATION, EVALUATION AND OTHER MATTERS RELATING TO STUDENTS"
section_number: "99"
---


<article class="act-document act-section-document">
 <p class="act-chapter-kicker">CHAPTER VII: ADMISSION, EXAMINATION, EVALUATION AND OTHER MATTERS RELATING TO STUDENTS</p>
 <h2 class="act-section-heading"><span>99</span>Sports and Extra- Curricular Activities</h2>
 <p class="act-essence-box">Brief: The University shall frame appropriate University Ordinance and Rules to ensure that the students selected to represent their Department, Colleges or the University, as the case may be, for sports, culture and all other extra-curricular activities are selected entirely on the basis of merit, through open merit competition alone and on no other basis. CHAPTER VIII: GRIEVANCES OF STUDENTS AND EMPLOYEE.</p>
 <div class="act-section-body">
The University shall frame appropriate University Ordinance and Rules to ensure that the students selected to represent their Department, Colleges or the University, as the case may be, for sports, culture and all other extra-curricular activities are selected entirely on the basis of merit, through open merit competition alone and on no other basis.
CHAPTER VIII: GRIEVANCES OF STUDENTS AND EMPLOYEE
  </div>
</article>

<nav class="act-page-nav" aria-label="Act page navigation">
 <a class="act-page-nav-link" href="{{ '/jharkhand-university-act/098-examination-and-evaluation-not-invalid-for-non-compliance-with-schedul/' | relative_url }}">Previous</a>
 <a class="act-page-nav-link act-page-nav-toc" href="{{ '/jharkhand-state-universities-act-2026/' | relative_url }}">TOC</a>
 <a class="act-page-nav-link" href="{{ '/jharkhand-university-act/100-student-grievance-redressal-committee/' | relative_url }}">Next</a>
</nav>
