---
layout: "act"
title: "Prohibition of Prosecution"
act_part: "CHAPTER XV: JHARKHAND STATE UNIVERSITY SERVICE COMMISSION - Section 158"
order: 168
source_pdf: "/assets/pdf/ref/acts/2026-Gazette-Jharkhand-University-Act.pdf"
chapter_title: "CHAPTER XV: JHARKHAND STATE UNIVERSITY SERVICE COMMISSION"
section_number: "158"
---


<article class="act-document act-section-document">
 <p class="act-chapter-kicker">CHAPTER XV: JHARKHAND STATE UNIVERSITY SERVICE COMMISSION</p>
 <h2 class="act-section-heading"><span>158</span>Prohibition of Prosecution</h2>
 <div class="act-section-body">
<p class="act-line act-line-continuation">No prosecution for any offence under this Act shall be instituted without the sanction of an Officer or Authority as may be determined by the State Government, by an order, issued in this regard.</p>
  </div>
</article>

<nav class="act-page-nav" aria-label="Act page navigation">
 <a class="act-page-nav-link" href="{{ '/jharkhand-university-act/157-protection-for-action-taken-in-good-faith/' | relative_url }}">Previous</a>
 <a class="act-page-nav-link act-page-nav-toc" href="{{ '/jharkhand-state-universities-act-2026/' | relative_url }}">TOC</a>
 <a class="act-page-nav-link" href="{{ '/jharkhand-university-act/159-validation-of-proceedings-of-commission/' | relative_url }}">Next</a>
</nav>
